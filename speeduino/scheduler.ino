...
static inline void ignitionSchedule1Interrupt() //Most ARM chips can simply call a function
#endif
  {
    if (ignitionSchedule1.Status == PENDING) //Check to see if this schedule is turn on
    {
      ...
      // we ahave a fixed period here of 1ms for the intermediate phase.
      // Eventually thi will be a configuration for tacho pulse width
      // it'd 1000 microseconds which is of course 1ms.
    ...
    else if (ignitionSchedule1.Status == INTERMEDIATE)
    {
      ...
      // need to do this for all of the channels (other functions same as this one) not just channel 1.
      TACH_PULSE_HIGH();  // moved this from the end callback routine.
      ignitionSchedule1.Status = RUNNING; //Set the status to be in progress (ie The start callback has been called, but not the end callback)
...
static inline void ignitionSchedule2Interrupt() //Most ARM chips can simply call a function
#endif
  {
    ...
    else if (ignitionSchedule2.Status == INTERMEDIATE)
    {
      ...
      // as we've already had 1ms we must remove this amount from the next timer compare value we assign
      if(ignitionSchedule2.endScheduleSetByDecoder == true) { IGN2_COMPARE = ignitionSchedule2.endCompare; } 
...
static inline void ignitionSchedule5Interrupt() //Most ARM chips can simply call a function
#endif
  {
    if (ignitionSchedule5.Status == PENDING) //Check to see if this schedule is turn on
    {
      ...
      ignitionSchedule5.Status = INTERMEDIATE; //Set the status to be in progress (ie The start callback has been called, but not the end callback)
